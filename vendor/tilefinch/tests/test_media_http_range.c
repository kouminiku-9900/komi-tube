...
static void test_video_lookahead_policy(void)
{
    ...
    CHECK(psp_media_video_lookahead_limit(
              true, PSP_MEDIA_LOOKAHEAD_AUDIO_RESERVE_US - 1u) == 1u);
    CHECK(psp_media_video_lookahead_limit(
              true, PSP_MEDIA_LOOKAHEAD_AUDIO_RESERVE_US) == 2u);
...
static void test_buffering_policy_hysteresis(void)
{
    ...
    /* A high-bitrate stream whose fixed windows are full cannot reach the
       startup target; with nothing left to fetch it starts from the ordinary
       target rather than waiting forever. */
    input.ready_since_us = 0;
    input.network_ahead_us = PSP_MEDIA_BUFFER_TARGET_US;
    input.fill_pending = false;
    decision = psp_media_buffer_policy(input);
    CHECK(decision.ready_since_us == input.now_us);
    input.network_ahead_us = PSP_MEDIA_BUFFER_TARGET_US - 1u;
    input.ready_since_us = 0;
    decision = psp_media_buffer_policy(input);
    CHECK(decision.ready_since_us == 0);
    input.fill_pending = true;
