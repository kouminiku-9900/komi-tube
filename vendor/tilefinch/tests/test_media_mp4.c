...
int main(void)
{
    ...
    puts("test: the Media Engine pool hands out visible, aligned storage");
    {
        ...
        CHECK(!psp_media_pool_reservation_admitted(0)
              ...
              /* A heap too small to keep pages usable beside the pool. */
              && !psp_media_pool_reservation_admitted(
                     8u * 1024u * 1024u)
              /* A stock-partition heap now reserves too: the pool is what
                 keeps the 4 MiB-aligned DDR arena available after the heap
                 has fragmented. */
              && psp_media_pool_reservation_admitted(
                     22u * 1024u * 1024u)
              && !psp_media_pool_reservation_admitted(
