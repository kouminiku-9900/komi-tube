...
static bool resolves_youtube(const char *input, const char *expected)
{
    char output[512];
    return browser_omnibox_resolve_youtube(input, output, sizeof(output))
        && strcmp(output, expected) == 0;
}

int main(void)
{
    CHECK(resolves_youtube(
        " psp game ",
        "https://m.youtube.com/results?search_query=psp%20game"));
    CHECK(resolves_youtube(
        "\xe3\x83\x9e\xe3\x83\xaa\xe3\x82\xaa",
        "https://m.youtube.com/results?search_query="
        "%E3%83%9E%E3%83%AA%E3%82%AA"));
    CHECK(resolves_youtube(
        "en.wikipedia.org/wiki/PSP",
        "https://m.youtube.com/results?search_query="
        "en.wikipedia.org%2Fwiki%2FPSP"));
    CHECK(resolves_youtube(
        "https://www.youtube.com/watch?v=M_t6TpjIjHc",
        "https://www.youtube.com/watch?v=M_t6TpjIjHc"));
    CHECK(resolves_youtube(
        "youtu.be/M_t6TpjIjHc", "https://youtu.be/M_t6TpjIjHc"));
    {
        char output[512];
        CHECK(!browser_omnibox_resolve_youtube("   ", output, sizeof(output)));
    }
    CHECK(resolves(
