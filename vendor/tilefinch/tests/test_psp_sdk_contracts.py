...
class PspSdkContractTests(unittest.TestCase):
    ...
    def test_ordinary_boot_skips_synthetic_clock_work_and_splash_waits(self):
        ...
        # The second entrance repaints after the boot-time system Wi-Fi
        # dialog, which leaves the display switched off.
        self.assertEqual(len(list(call_arguments(
            main, "psp_present_boot_entrance"))), 2)
    ...
    def test_media_engine_buffers_come_from_a_boot_time_pool(self):
        ...
        # Exactly three allocation calls in the whole backend: the boot
        # reservation, the first-open fallback reservation (which refuses an
        # address above the Media Engine's reach), and the one helper that
        # draws from the pool. Anything else would be a buffer that can land
        # above the Media Engine's reach.
        self.assertEqual(backend.count("memalign("), 3)
        ensure = backend[backend.index("static void psp_media_pool_ensure("):]
        ensure = ensure[:ensure.index("\n}\n") + 3]
        self.assertIn("psp_media_me_invisible(", ensure)
        reserve = backend[
    ...
    def test_codec_worker_death_and_retry_quarantine_are_immediate(self):
        ...
        self.assertIn("Restart komi-tube to use video", ui)
    ...
    def test_video_open_prioritizes_media_and_does_not_wait_for_readahead(self):
        ...
        self.assertLess(
            ...
            route.index("psp_app_focus_memory_on_video("))
    ...
    def test_provider_handoff_retires_page_work_before_resolver_progress(self):
        ...
        # YouTube for PSP gives the chosen video the memory immediately: the
        # focus helper retires page requests and drops every optional cache
        # in one step before the resolver asks for its response buffer.
        self.assertLess(
            opened.index("psp_app_focus_memory_on_video("),
            opened.index("provider_handoff_present_pending ="))
        youtube = without_comments(
            (ROOT / "src/psp_app/psp_app_youtube.c").read_text(
                encoding="utf-8"))
        focus = youtube[
            youtube.index("void psp_app_focus_memory_on_video("):]
        self.assertIn("browser_engine_cancel_network_work(", focus)
        self.assertIn("browser_engine_reclaim_optional_memory(", focus)
        self.assertIn("psp_youtube_preresolve_reset(", focus)
    ...
    def test_every_process_exit_hands_the_console_back(self):
        ...
        self.assertEqual(
            ...
            sorted([
                ...
                # Declining the boot Wi-Fi dialog exits before the
                # configuration (and so any exit_to) has been loaded.
                "NULL",
            ]),
