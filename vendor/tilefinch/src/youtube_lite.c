...
/* Long enough that searches keep using the compact API path for a whole
   session; a rejected identity still falls back to the HTML page, which
   refreshes it. */
#define YOUTUBE_LITE_IDENTITY_CACHE_MAX_AGE_NS \
    (UINT64_C(6) * 60u * 60u * UINT64_C(1000000000))
    ...
    const YoutubeLiteSpan *renderer, YoutubeLiteWatch *watch)
{
    ...
    /* The next-API watch path carries no videoDetails; its header renderer
       is then the only source of the title and channel. */
    if (watch->video.title[0] == '\0')
        (void) lite_text_runs(renderer, "title", watch->video.title,
                              sizeof(watch->video.title));
    if (watch->video.channel[0] == '\0')
        (void) lite_text_runs(renderer, "channel", watch->video.channel,
                              sizeof(watch->video.channel));
    if (lite_text_runs(
    ...
    BrowserSession *session, YoutubeLiteIdentity *identity)
{
    ...
    /* Unbound: the transport applies response cookies after the job has
       stored the identity, so a cookie-bound entry was stale before the next
       search could read it and the compact API path was never taken. A
       rejected identity falls back to the HTML page, which refreshes it. */
    return browser_session_site_adapter_state_get_unbound(
...
/*
 * Built-in anonymous MWEB identity: a recent client version and no API key
 * (the youtubei endpoints answer anonymous requests without one). It lets
 * every route use the compact JSON APIs without first downloading an HTML
 * page just to read these values. A rejected request falls back to the HTML page, which then caches
 * the live identity.
 */
static void lite_default_identity(YoutubeLiteIdentity *identity)
{
    memset(identity, 0, sizeof(*identity));
    identity->version = YOUTUBE_LITE_IDENTITY_CACHE_VERSION;
    snprintf(identity->client_version, sizeof(identity->client_version),
             "%s", "2.20260918.00.00");
}
...
struct YoutubeLiteLoadJob {
    ...
    /* Watch page built from the compact next API instead of watch HTML. */
    bool direct_watch;
...
static bool lite_prepare_supplemental_request(
    ...
    const char *language, bool comments, bool watch_view,
    const char *prepared_token,
    YoutubeLiteSupplementalRequest *prepared)
{
    ...
    /* YouTube for PSP: an initial search (no continuation token) asks the
       search API directly with the query. Its MWEB JSON is ~150-190 KiB,
       against ~580 KiB of hex-escaped HTML plus a second decode buffer that
       could not be allocated beside the PSP media pool. */
    char query[YOUTUBE_LITE_QUERY_LIMIT] = {0};
    if (watch_view) {
        /* Watch page and related videos from the next API (~160-215 KiB of
           JSON) instead of ~750 KiB of watch HTML plus a decode buffer. */
        comments = false;
        token[0] = '\0';
        if (!youtube_watch_url_video_id(page_url, query)) return false;
    } else if (!comments && token[0] == '\0'
        && !lite_query_value(page_url, "search_query", query, sizeof(query)))
        return false;
    int endpoint_length = snprintf(
        prepared->endpoint, sizeof(prepared->endpoint),
        "https://m.youtube.com/youtubei/v1/%s?%s%s%sprettyPrint=false",
        comments || watch_view ? "next" : "search",
        identity->api_key[0] == '\0' ? "" : "key=", identity->api_key,
        identity->api_key[0] == '\0' ? "" : "&");
    ...
    if (endpoint_length < 0
        ...
        || !lite_json_quote(token[0] != '\0' ? token : query,
                            token_json, sizeof(token_json))
    ...
    int body_length = snprintf(
        ...
        "\"clientVersion\":\"%s\",\"hl\":\"%s\",\"gl\":\"%s\","
        "\"visitorData\":%s}},\"%s\":%s}",
        identity->client_version, language,
        strncmp(language, "ja", 2) == 0 ? "JP" : "US", visitor_json,
        token[0] != '\0' || comments ? "continuation"
            : watch_view ? "videoId" : "query",
        token_json);
    ...
    char *error, size_t error_size)
{
    ...
    /* Read the cached identity before the preference cookies are rewritten.
       The cache is bound to the cookie jar's fingerprint, and re-setting
       PREF/SOCS moves their jar slots even when the values are unchanged,
       which invalidated the identity on every job and kept every search on
       the ~580 KiB HTML path. */
    bool identity_cached = route == YOUTUBE_LITE_ROUTE_SEARCH
    ...
    if (!identity_cached) {
        lite_default_identity(&job->identity);
        identity_cached = true;
    }
    if (route == YOUTUBE_LITE_ROUTE_HOME) {
        /* Home is only the search surface; it needs no YouTube data. */
        job->primary_status_code = 200;
        job->phase = YOUTUBE_LITE_JOB_BUILD;
        return job;
    }
    job->direct_watch = route == YOUTUBE_LITE_ROUTE_WATCH
        && !job->supplemental_requested
        && !lite_description_view_requested(url);
    bool can_skip_primary = identity_cached
        && (route == YOUTUBE_LITE_ROUTE_SEARCH || job->direct_watch);
    if (can_skip_primary) {
...
static bool lite_load_enqueue_supplemental(YoutubeLiteLoadJob *job)
{
    ...
    if (!lite_prepare_supplemental_request(
            ...
            job->language, comments && !job->direct_watch, job->direct_watch,
            job->comments_scan.token, prepared)) {
    ...
    if (job->phase == YOUTUBE_LITE_JOB_BUILD) {
        ...
        if (!job->watch_facts_reused && job->route == YOUTUBE_LITE_ROUTE_WATCH
            ...
            (void) lite_comments_scan_pump(job->decoded, job->decoded_length,
                                           &job->comments_scan);
        if (job->build == NULL && job->direct_watch
            && job->supplemental_fetched) {
            /* The next-API response is the watch page's whole data. */
            job->build = lite_build_work_create(
                job->budget, job->url, NULL, 0, 0, NULL, 0,
                job->supplemental->data, job->supplemental->length,
                job->compact_results, true,
                job->error, sizeof(job->error));
