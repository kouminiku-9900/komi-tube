    ...
    char *output, size_t capacity)
{
    ...
    int system_language = PSP_SYSTEMPARAM_LANGUAGE_ENGLISH;
    sceUtilityGetSystemParamInt(
        PSP_SYSTEMPARAM_ID_INT_LANGUAGE, &system_language);
    bool japanese = system_language == PSP_SYSTEMPARAM_LANGUAGE_JAPANESE;
    data.language = japanese ? PSP_UTILITY_OSK_LANGUAGE_JAPANESE
                             : PSP_UTILITY_OSK_LANGUAGE_DEFAULT;
    data.lines = 1;
    data.unk_24 = 1;
    /* INPUTTYPE_ALL does not enable the firmware's kana-to-kanji converter.
       Name every Japanese mode explicitly (SELECT cycles through them) so
       search text can contain kanji, plus the Latin modes for romaji and
       digits. */
