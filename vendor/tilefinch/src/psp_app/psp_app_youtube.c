...
/*
 * YouTube for PSP: when a video is chosen, give it the memory. The result
 * page stays committed as the rollback surface, but everything optional the
 * browser holds -- staged fonts, script heap slack, the HTTP response cache
 * and cached render tiles -- is dropped at once rather than one phase per
 * frame, and in-flight page requests, preconnects and any stale
 * pre-resolution are retired. On a stock-memory PSP the second video's
 * resolver otherwise could not obtain its ~256 KiB player response buffer
 * ("player: response exceeded shared memory budget"). The page repaints
 * from its document when the player closes.
 */
void psp_app_focus_memory_on_video(
    BrowserEngine *engine, PspYoutubePreresolve *preresolve,
    const char *reason)
{
    if (reason == NULL) reason = "video selected";
    psp_youtube_preresolve_reset(preresolve, reason);
    size_t cancelled = browser_engine_cancel_network_work(engine, reason);
#ifdef TILEFINCH_PSP_LIVE_NETWORK
    fetch_preconnect_cancel(reason);
#endif
    BrowserOptionalMemoryReclaim reclaim = {0};
    (void) browser_engine_reclaim_optional_memory(engine, &reclaim);
    printf("tilefinch-video-focus: reason=\"%.32s\" cancelled=%zu "
           "fonts=%zu js=%zu cache=%zu render=%zu total=%zu\n",
           reason, cancelled, reclaim.font_staging_bytes,
           reclaim.javascript_bytes, reclaim.session_cache_bytes,
           reclaim.render_cache_bytes, reclaim.total_bytes);
}
