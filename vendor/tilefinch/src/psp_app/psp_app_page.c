...
/*
 * YouTube for PSP: every page load starts from a clean slate. After several
 * videos the heap is fragmented, and a YouTube page body buffered while the
 * previous page's optional caches were still resident failed mid-transfer
 * ("stream consumer rejected background response body"). Dropping staged
 * fonts, script slack, the HTTP cache and render tiles first costs a refetch
 * of thumbnails on Back, which the app accepts in exchange for reliability.
 */
void psp_reclaim_before_navigation(BrowserEngine *engine, const char *kind)
{
    BrowserOptionalMemoryReclaim reclaim = {0};
    if (!browser_engine_reclaim_optional_memory(engine, &reclaim)) return;
    printf("tilefinch-navigation-reclaim: kind=%s reclaimed=%zu\n",
           kind == NULL ? "page" : kind, reclaim.total_bytes);
}
...
{
    psp_reclaim_before_navigation(engine, "url");
...
{
    psp_reclaim_before_navigation(engine, "action");
