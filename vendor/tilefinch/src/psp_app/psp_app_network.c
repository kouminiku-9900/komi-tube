                                ...
                                const uint16_t *frame, PspUiState *ui)
{
    ...
    if (psp_network_lifecycle_needs_demand_recovery(
        ...
        if (psp_bound_network_lifecycle != NULL
            && psp_bound_network_lifecycle->machine.state
                == PSP_NETWORK_SUPERVISOR_OFFLINE)
            psp_ui_show_status(ui, "WIFI OFFLINE - OPEN PSP NETWORK SETTINGS", 600);
    ...
    if (!adopted && !psp_network_begin(network, profile_index)) {
        ...
        psp_log_checkpoint("network-begin-refused");
        {
            ...
            snprintf(status, sizeof(status),
                     "WIFI PROFILE ERROR P%d - OPEN PSP NETWORK SETTINGS",
                     profile_index);
            psp_ui_show_status(ui, status, 600);
        }
        return false;
    }
    if (adopted) {
    ...
    /* Let the firmware's own connection dialog perform association unless
       an adopted warmup has already asked APCTL to join. */
    if (!network->connect_started) network->connect_via_dialog = true;
    psp_ui_set_loading(ui, true, -1);
    psp_ui_show_status(ui, "PREPARING WI-FI - CIRCLE CANCELS", 600);
    psp_navigation_cooperate_begin(ui, frame, NULL);
    ...
    for (;;) {
        ...
        if (stage_changed || periodic) {
            ...
            char status[80];
            snprintf(status, sizeof(status), "WI-FI %s - CIRCLE CANCELS",
                     psp_network_status_name(network->status));
            if (presentation_due) {
        ...
        if (terminal || network->dialog_pending) break;
    }
    psp_navigation_cooperate_end("network");
    while (network->dialog_pending) {
        /* The dialog takes the GE and both scanout buffers; no cooperate
           supervisor may paint while it runs. */
        int dialog_result = psp_network_run_connect_dialog();
        PspNetworkStatus outcome =
            psp_network_finish_dialog(network, dialog_result);
        printf("tilefinch-network-dialog: outcome=%s apctl=%d "
               "profile=%d native=0x%08x\n",
               psp_network_status_name(outcome), network->apctl_state,
               network->profile_index, (unsigned) network->native_result);
        if (outcome == PSP_NETWORK_READY) break;
        psp_ui_set_loading(ui, false, 0);
        psp_ui_show_status(
            ui, outcome == PSP_NETWORK_CANCELLED
                    ? "WI-FI NOT CONNECTED - X RETRY  O GIVE UP"
                    : "WI-FI FAILED - X RETRY  O GIVE UP",
            36000);
        if (frame != NULL) psp_present(frame, ui);
        /* Wait for a fresh press: the button that closed the dialog may
           still be held. */
        uint32_t previous_buttons = UINT32_MAX;
        bool retry = false;
        for (;;) {
            psp_log_heartbeat();
            sceDisplayWaitVblankStart();
            if (!psp_lifecycle_presentation_allowed(&psp_lifecycle)) break;
            SceCtrlData pad = {0};
            uint32_t buttons = sceCtrlPeekBufferPositive(&pad, 1) > 0
                ? pad.Buttons : 0u;
            uint32_t pressed = previous_buttons == UINT32_MAX
                ? 0u : buttons & ~previous_buttons;
            previous_buttons = buttons;
            if ((pressed & PSP_CTRL_CROSS) != 0) {
                retry = true;
                break;
            }
            if ((pressed & PSP_CTRL_CIRCLE) != 0) break;
        }
        if (!retry) break;
        network->dialog_pending = true;
        network->status = PSP_NETWORK_CONNECTING;
        psp_ui_show_status(ui, "OPENING WI-FI SETTINGS", 120);
        if (frame != NULL) psp_present(frame, ui);
    }
    ...
    if (!ready) {
        ...
        const char *status;
        if (cancelled) {
            status = "WI-FI NOT CONNECTED - START TO TRY AGAIN";
        } else if (network->failure_phase == PSP_NETWORK_LOADING_COMMON
                   && network->native_result == -2) {
            status = "WI-FI SWITCH IS OFF - TURN IT ON AND RETRY";
        } else if (saved) {
            status = "WI-FI FAILED - DETAILS SAVED IN DATA FOLDER";
        } else {
            status = "WI-FI FAILED - CHECK PSP NETWORK SETTINGS";
        }
        psp_ui_show_status(ui, status, 900);
    }
    psp_ui_set_loading(ui, ready, ready ? -1 : 0);
    if (ready) psp_ui_show_status(ui, "WI-FI CONNECTED - OPENING YOUTUBE", 120);
    if (frame != NULL) psp_present(frame, ui);
    return ready;
}
