    ...
    PspMediaHlsContext *context, char *error, size_t error_size)
{
    ...
    if (media_hls_playlist_kind(playlist) == MEDIA_HLS_PLAYLIST_MASTER) {
        ...
        if (!selected_ok) {
            ...
            char variants[160];
            media_hls_playlist_describe(playlist, variants, sizeof(variants));
            media_hls_playlist_destroy(playlist);
            char reason[256];
            snprintf(reason, sizeof(reason), "%s: %s",
                     context->opening_source != 0u
                         ? "nested HLS audio master is unsupported"
                         : has_video
                             ? "HLS master is missing its selected audio "
                               "rendition"
                             : "HLS master has no compatible 240p AVC "
                               "rendition",
                     variants);
            printf("tilefinch-media-hls: event=master-rejected %s\n", reason);
            return psp_hls_open_fail(context, reason, error, error_size);
        }
        media_hls_playlist_destroy(playlist);
