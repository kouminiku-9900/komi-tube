...
void media_hls_playlist_describe(
    const MediaHlsPlaylist *playlist, char *output, size_t output_size)
{
    if (output == NULL || output_size == 0) return;
    output[0] = '\0';
    if (playlist == NULL) return;
    size_t variants = 0, compatible = 0, audio = 0, used = 0;
    for (size_t i = 0; i < playlist->entry_count; i++) {
        const HlsEntry *entry = &playlist->entries[i];
        if (entry->kind == HLS_ENTRY_AUDIO_RENDITION) {
            audio++;
            continue;
        }
        if (entry->kind != HLS_ENTRY_VARIANT) continue;
        variants++;
        if (entry->codecs_compatible) compatible++;
        if (used + 24u < output_size) {
            int written = snprintf(
                output + used, output_size - used, "%s%ux%u%s",
                used == 0 ? "" : ",", (unsigned) entry->width,
                (unsigned) entry->height,
                entry->codecs_compatible ? "" : "!");
            if (written > 0) used += (size_t) written;
        }
    }
    char summary[64];
    snprintf(summary, sizeof(summary), " variants=%zu avc-aac=%zu audio=%zu",
             variants, compatible, audio);
    if (used + strlen(summary) < output_size)
        memcpy(output + used, summary, strlen(summary) + 1u);
}
