 ...
 */
/*
 * YouTube for PSP: the reservation is also the only defence against heap
 * fragmentation. Without it the 2 MiB, 4 MiB-aligned DDR arena is memaligned
 * from the live heap on every open; the first video usually succeeds and the
 * next one fails with "aligned codec allocation failed: missing=ddr
 * pool=none" once browsing has fragmented the heap. The app renders only
 * lightweight native YouTube pages, so a stock ~22 MB heap can afford to hold
 * the pool for the whole process. A stock-memory PSP leaves this process only
 * about 11 MB of heap, so keep just 3 MiB for pages at boot; when even that
 * does not fit, the backend reserves the pool lazily at the first video open
 * and keeps it (media_backend_psp.c, psp_media_pool_ensure).
 */
#define PSP_MEDIA_POOL_CONTENT_HEADROOM_BYTES ((size_t) (3u * 1024u * 1024u))
