 ...
 */
/*
 * Reserve the pool at the first open when boot could not afford it. The first
 * video on a stock-memory PSP used to succeed from a fresh heap and every
 * later one fail once the heap had fragmented; holding the reservation from
 * that first success onward keeps later opens on the same storage. A base the
 * Media Engine cannot address is released and the heap path continues.
 */
static bool psp_media_pool_lazy_attempted;

static void psp_media_pool_ensure(void)
{
    if (psp_media_pool.base != NULL || psp_media_pool_lazy_attempted) return;
    psp_media_pool_lazy_attempted = true;
    void *base = memalign(PSP_MEDIA_DDR_ALIGNMENT, PSP_MEDIA_POOL_BYTES);
    if (base != NULL && psp_media_me_invisible((uintptr_t) base)) {
        free(base);
        base = NULL;
    }
    if (base != NULL)
    ...
    psp_media_log(
        "tilefinch-media-decoder: event=me-pool-lazy base=0x%08X bytes=%u "
        "free=%u largest=%u",
    ...
    MediaBackend *backend_out, char *error, size_t error_size)
{
    ...
    if ((backend->have_video
        ...
        psp_media_error(
            error, error_size,
            "PSP aligned codec allocation failed: missing=%s%s%s%s%s%s "
            "pool=%s used=%u/%u free=%u largest=%u",
            backend->ddr_memory == NULL && backend->have_video ? "ddr," : "",
            (backend->surfaces[0] == NULL || backend->surfaces[1] == NULL)
                && backend->have_video ? "surface," : "",
            backend->mpeg_memory == NULL && backend->have_video
                ? "mpeg," : "",
            backend->packet_staging == NULL ? "packet," : "",
            backend->have_audio && backend->audio_codec == NULL
                ? "audio," : "",
            backend->have_video && backend->video_au == NULL ? "au" : "",
            psp_media_pool.base == NULL ? "none"
                : psp_media_pool.poisoned ? "poisoned" : "ok",
            (unsigned) psp_media_pool.cursor,
            (unsigned) psp_media_pool.bytes,
            (unsigned) sceKernelTotalFreeMemSize(),
            (unsigned) sceKernelMaxFreeMemSize());
        psp_media_destroy(backend);
        return false;
