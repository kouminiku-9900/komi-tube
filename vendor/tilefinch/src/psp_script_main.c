    ...
    const PspFailureReportData *diagnostics)
{
    ...
    /* Keep every failure, not only the latest, so a session with several
       different errors can be diagnosed afterwards. Bounded: restart the
       history once it passes 64 KiB. */
    /* The partition figures above describe the kernel partition, which the
       heap has already consumed. Record what the heap itself can still hand
       out: the largest single block, probed by halving, which separates a
       fragmented heap from an exhausted one. */
    size_t heap_largest = 0;
    for (size_t step = 8u * 1024u * 1024u; step >= 4096u; step /= 2u) {
        void *probe = malloc(heap_largest + step);
        if (probe != NULL) {
            free(probe);
            heap_largest += step;
        }
    }
    char history[TILEFINCH_INSTALL_PATH_LIMIT];
    if (tilefinch_install_data_path(
            psp_failure_report_paths, "tilefinch-error-history.txt",
            history, sizeof(history))) {
        struct stat info;
        if (stat(history, &info) == 0 && info.st_size > 64 * 1024)
            (void) remove(history);
        FILE *log = fopen(history, "ab");
        if (log != NULL) {
            (void) fprintf(
                log,
                "%04u-%02u-%02uT%02u:%02u:%02uZ uptime=%llums stage=%s "
                "http=%ld native=0x%08x heap-largest=%zu "
                "url=%.300s detail=%.1000s\n",
                ...
                stage == NULL ? "unknown" : stage, http_status,
                (unsigned) native_result, heap_largest,
...
PSP_MODULE_INFO("YouTubePSP", 0, 0, 1);
    ...
    char *destination, size_t destination_capacity)
{
    ...
    bool accepted = use_voice
        ...
        : psp_text_input_request(
              text_input, engine_frame, ui, &request,
              address, sizeof(address));
    return accepted && browser_omnibox_resolve_youtube(
        address, destination, destination_capacity);
}
        ...
        if (deferred_url != NULL) {
...
#endif
            {
                /* Same order as activating a HOME tile. */
            ...
            if (deferred_started) {
                ...
                if (restoring_last_page)
                    psp_ui_show_status(&process->presentation.ui,
                                       "RESTORING LAST PAGE", 180);
            } else {
...
&& defined(TILEFINCH_PSP_LIVE_NETWORK)
        ...
        if (!media_open_was_pending
            ...
            psp_app_focus_memory_on_video(
                browser->engine, &browser->youtube_preresolve,
                "video selected");
        }
        psp_log_set_phase(PSP_LOG_PHASE_MEDIA);
        psp_log_heartbeat();
        ...
        if (network_was_warming
            ...
            if (network->status == PSP_NETWORK_READY
                ...
                psp_report_network_result(network);
                {
                    char status[112];
                    snprintf(status, sizeof(status),
                             "WIFI FAILED P%d - OPEN PSP NETWORK SETTINGS",
                             (int) process->config.network_profile);
                    psp_ui_show_status(&process->presentation.ui, status, 600);
                }
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    psp_present_boot_entrance(0u, "STARTING YOUTUBE PSP", false, NULL);
...
#ifdef TILEFINCH_PSP_LIVE_NETWORK
    /*
     * Connect through the PSP's own Wi-Fi screen before the browser starts.
     * This is the same firmware dialog games use: the user picks any saved
     * connection and the firmware reports its own errors. The network ladder
     * later adopts the connected services instead of calling APCTL with a
     * guessed profile number, which on hardware could sit in DISCONNECTED
     * for the whole timeout with no explanation on screen.
     */
    int boot_wifi_profile = 0;
    PspBootConnectResult boot_wifi =
        psp_network_boot_connect(&boot_wifi_profile);
    if (boot_wifi != PSP_BOOT_CONNECT_READY) {
        printf("tilefinch-boot-wifi: result=%d exiting\n", (int) boot_wifi);
        psp_log_finish("wifi-declined");
        /* Configuration is not loaded yet, so there is no exit_to. */
        psp_exit_console(NULL);
        return 0;
    }
    psp_present_boot_entrance(0u, "WI-FI CONNECTED - STARTING", false, NULL);
#endif
#if defined(TILEFINCH_PSP_VALIDATION_LOG) \
    && defined(TILEFINCH_PSP_LIVE_NETWORK)
    ...
    psp_boot_config_disable_automation(&process.config);
#ifdef TILEFINCH_PSP_LIVE_NETWORK
    /* Rejoin after sleep must reconnect to what the user picked. */
    if (boot_wifi_profile > 0)
        process.config.network_profile = boot_wifi_profile;
#endif
#endif
    if (!config_loaded)
    ...
    if (restoring_last_page && !native_home_boot) {
        ...
        /* YouTube for PSP: HOME has one useful tile. Open it for the user
           exactly as pressing it would, once the browser is interactive,
           instead of stopping on the launcher (CONTINUE / NOTHING OPEN). */
        (void) psp_boot_queue_request(&boot_queue, "https://m.youtube.com/");
    }
    ...
    if (loaded) {
        ...
        static const char failure_html[] =
            ...
            "<h1>Page unavailable</h1><p>komi-tube could not open this page."
