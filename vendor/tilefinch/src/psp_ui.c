    ...
    char *detail, size_t detail_capacity)
{
    ...
    if (strncmp(at, "https://", 8u) == 0) at += 8;
        ...
        copy_string(domain, domain_capacity, "YouTube");
...
void psp_ui_init(PspUiState *ui)
{
    ...
    copy_string(ui->title, sizeof(ui->title), "YouTube");
    ...
    int stride, uint16_t accent, uint16_t text, uint16_t muted)
{
    ...
    if (ui->menu_selection == 2u) {
        ...
        lines[0] = "komi-tube " TILEFINCH_VERSION_STRING;
        lines[1] = "Sony PSP release build";
        lines[2] = "480 x 272  |  333 MHz target";
    } else {
        title = "Licences";
        lines[0] = "komi-tube includes open-source code.";
    ...
    int stride, uint16_t accent, uint16_t text, uint16_t muted)
{
    ...
    if (ui->update_history_phase == TILEFINCH_UPDATE_HISTORY_LOADING) {
        ...
        for (size_t row = 0; row < visible; row++) {
            ...
            char label[24];
            snprintf(label, sizeof(label), "komi-tube %s",
    ...
    int stride, uint16_t accent)
{
    ...
    if (view == NULL || view->modules == NULL
        ...
        draw_text(pixels, width, height, stride,
                  box.x + 18, box.y + 54,
                  "Build a QR report from this app's", 42,
    ...
    snprintf(version, sizeof(version), "komi-tube %s", view->app_version);
                               ...
                               UiHomeEntrance entrance)
{
    ...
    if (entrance.furniture != 0u)
        draw_surface_status(
            pixels, width, height, stride, accent.accent, "YouTube",
            entrance.furniture);
                       ...
                       uint16_t accent, uint16_t text)
{
    ...
    static const char *const tls_guidance[] = {
        ...
        "Try another Wi-Fi or update komi-tube",
        "Secure connection was redirected. Try another Wi-Fi network",
        "Secure connection unsupported. Update komi-tube",
...
#define MARK(value) ((value) * size / TILEFINCH_BOOT_MARK_UNITS)
    ...
    fill_round_rect(
        pixels, width, height, stride,
        (UiRect) {x, y + MARK(7), MARK(48), MARK(34)}, MARK(8), red, 4);
    fill_triangle(
        pixels, width, height, stride,
        x + MARK(20), y + MARK(14),
        x + MARK(20), y + MARK(34),
        x + MARK(34), y + MARK(24), white);
#undef MARK
}
    ...
    int width, int height, int stride)
{
    ...
    draw_text(
        pixels, width, height, stride, 16, 53, "YouTube", 9,
        heading, 2);
    ...
    draw_text(
        ...
        "SEARCH YOUTUBE", 14, muted, 1);

    const UiRect cards[1] = {
        {16, 119, width - 32, 64}
    };
    const char *names[1] = {"YOUTUBE"};
    const char *descriptions[1] = {
        "SEARCH AND PLAY VIDEO"
    };
    for (size_t at = 0; at < 1; at++) {
        fill_round_rect(
    ...
    psp_ui_set_page(&shell, "YouTube", "tilefinch://home", false);
        ...
        if (media->resolving) {
            ...
            if (media->retry_unavailable) {
                ...
                draw_text_with_font(
                    ...
                    "Restart komi-tube to use video", 30,
