...
static bool youtube_host(const TilefinchUrl *url)
{
    static const char *const hosts[] = {
        "youtube.com", "www.youtube.com", "m.youtube.com", "youtu.be"
    };
    for (size_t i = 0; i < sizeof(hosts) / sizeof(hosts[0]); i++) {
        size_t length = strlen(hosts[i]);
        if (url->host_length == length
            && strncasecmp(url->value + url->host_offset, hosts[i], length)
                   == 0) return true;
    }
    return false;
}

bool browser_omnibox_resolve_youtube(
    const char *input, char *output, size_t output_capacity)
{
    if (output == NULL || output_capacity == 0) return false;
    output[0] = '\0';
    if (input == NULL) return false;
    char navigation[TILEFINCH_URL_SERIALIZED_LIMIT];
    BrowserOmniboxResolutionKind kind = BROWSER_OMNIBOX_SEARCH;
    if (browser_omnibox_resolve_kind(
            input, BROWSER_SEARCH_GOOGLE, navigation, sizeof(navigation),
            &kind)
        && kind == BROWSER_OMNIBOX_NAVIGATION) {
        TilefinchUrl parsed;
        if (tilefinch_url_parse(navigation, &parsed) && youtube_host(&parsed)) {
            size_t length = strlen(navigation);
            if (length >= output_capacity) return false;
            memcpy(output, navigation, length + 1u);
            return true;
        }
    }
    while (isspace((unsigned char) *input)) input++;
    ...
    bool resolved = append_encoded_query(
        ...
        "https://m.youtube.com/results?search_query=", input, length);
    if (!resolved) output[0] = '\0';
    return resolved;
}
