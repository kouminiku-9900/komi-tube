...
{
    return site_adapter_state_get(
        session, key, data, data_capacity, data_length, now_ns,
        maximum_age_ns, true);
}

bool browser_session_site_adapter_state_get_unbound(
    BrowserSession *session, const char *key, void *data,
    size_t data_capacity, size_t *data_length, uint64_t now_ns,
    uint64_t maximum_age_ns)
{
    ...
    uint64_t maximum_age_ns, bool cookie_bound)
{
    ...
    if (!state->valid || strcmp(state->key, key) != 0
        ...
        || (cookie_bound && state->cookie_fingerprint
               != session_cookie_fingerprint(session))) {
        return false;
    }
