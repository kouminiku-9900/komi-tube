    ...
    YoutubeResolveJob *job, size_t additional, size_t maximum_bytes)
{
    ...
    /* One up-front block large enough for a typical ~250 KiB player
       response, then fixed steps. Doubling from 64 KiB needed the old and new
       blocks together at every copy, which a fragmented PSP heap could not
       supply for a second video. */
    size_t capacity = job->response.capacity == 0
        ? 384u * 1024u : job->response.capacity;
    while (capacity < required) {
        size_t next = capacity > maximum_bytes - 256u * 1024u
            ? maximum_bytes + 1u : capacity + 256u * 1024u;
    ...
    YoutubeResolvePhase parse_phase)
{
    ...
    if (progress.available_body_bytes != 0) {
        if (!youtube_resolve_job_reserve(
            ...
            const Budget *b = job->budget;
            const size_t kib = 1024u;
            youtube_resolve_job_fail(
                job, "player: response exceeded shared memory budget"
                     " (%s want=%zu len=%zu cap=%zu max=%zuK"
                     " budget=%zuK/%zuK ext=%zuK"
                     " dom/js/sty/res/lay/ren/ses/nav/oth="
                     "%zu/%zu/%zu/%zu/%zu/%zu/%zu/%zu/%zuK)",
                youtube_resolve_phase_name(job->phase),
                progress.available_body_bytes, job->response.length,
                job->response.capacity, maximum_bytes / kib,
                b->current / kib, b->limit / kib,
                b->external_reserved / kib,
                b->categories[BUDGET_CATEGORY_DOM].current / kib,
                b->categories[BUDGET_CATEGORY_JAVASCRIPT].current / kib,
                b->categories[BUDGET_CATEGORY_STYLE].current / kib,
                b->categories[BUDGET_CATEGORY_RESOURCE].current / kib,
                b->categories[BUDGET_CATEGORY_LAYOUT].current / kib,
                b->categories[BUDGET_CATEGORY_RENDER].current / kib,
                b->categories[BUDGET_CATEGORY_SESSION].current / kib,
                b->categories[BUDGET_CATEGORY_NAVIGATION].current / kib,
                b->categories[BUDGET_CATEGORY_UNCATEGORIZED].current / kib);
            return false;
        }
        size_t taken = 0;
        if (fetch_background_transport_take_chunk(
                ...
                job->response.capacity - job->response.length - 1u,
                &taken)) {
...
/*
 * The player configuration a watch page yields (visitor, API key and the
 * signature timestamp) is not per video. Remember the last complete set so
 * later videos go straight to the enriched player request instead of
 * streaming another 1.3+ MiB watch page (STS sits ~680 KiB in).
 */
...
static struct {
    ...
    char api_key[128];
    uint64_t signature_timestamp;
} youtube_watch_config_cache;

static void youtube_watch_config_cache_store(const YoutubeResolveJob *job)
{
    snprintf(youtube_watch_config_cache.visitor,
             sizeof(youtube_watch_config_cache.visitor), "%s", job->visitor);
    snprintf(youtube_watch_config_cache.api_key,
             sizeof(youtube_watch_config_cache.api_key), "%s", job->api_key);
    youtube_watch_config_cache.signature_timestamp = job->signature_timestamp;
    youtube_watch_config_cache.valid = true;
}
...
static bool youtube_resolve_job_take_watch_prefix(YoutubeResolveJob *job)
{
    ...
    if (!job->watch_have_visitor || !job->watch_have_api_key
        || !job->watch_have_signature_timestamp) {
        /* Only the overlap can still matter to the next scan. Keeping the
           whole page grew one buffer past 600 KiB per video, which a
           fragmented PSP heap refused on the second video. */
        if (job->response.length > 4u * YOUTUBE_WATCH_SCAN_OVERLAP) {
            size_t keep = YOUTUBE_WATCH_SCAN_OVERLAP;
            memmove(job->response.data,
                    job->response.data + job->response.length - keep, keep);
            job->watch_discarded_bytes += job->response.length - keep;
            job->response.length = keep;
            job->response.data[keep] = '\0';
            job->watch_scan_offset = keep;
        }
        return false;
    }
    youtube_watch_config_cache_store(job);
    job->watch_bytes = job->watch_discarded_bytes + job->response.length;
    job->watch_status = 0;
    uint64_t now = tilefinch_platform_monotonic_time_ns();
    uint64_t elapsed_us = now >= job->request_started_ns
        ? (now - job->request_started_ns) / UINT64_C(1000) : 0;
    ...
    YoutubeResolverCancelCallback cancel, void *cancel_opaque)
{
    ...
    if (youtube_watch_config_cache.valid) {
        snprintf(job->api_key, sizeof(job->api_key), "%s",
                 youtube_watch_config_cache.api_key);
        snprintf(job->visitor, sizeof(job->visitor), "%s",
                 youtube_watch_config_cache.visitor);
        job->signature_timestamp =
            youtube_watch_config_cache.signature_timestamp;
        job->enriched_from_cached_identity = true;
        job->phase = YOUTUBE_RESOLVE_PHASE_ENRICHED_START;
    } else if (youtube_lite_resolver_identity_get(session, &identity)) {
        snprintf(job->api_key, sizeof(job->api_key), "%s",
                 identity.api_key);
        snprintf(job->visitor, sizeof(job->visitor), "%s",
                 identity.visitor);
...
YoutubeResolveJobStatus youtube_resolve_job_pump(YoutubeResolveJob *job)
{
    ...
    case YOUTUBE_RESOLVE_PHASE_WATCH_PARSE:
        ...
        /* The streaming scan discards consumed bytes; finish it on the
           final window before judging the page. */
        if (job->response_ok && youtube_resolve_job_take_watch_prefix(job))
            break;
        if (!job->response_ok) {
            youtube_resolve_job_fail(
    ...
    case YOUTUBE_RESOLVE_PHASE_ENRICHED_PARSE: {
        ...
        if (!job->response_ok) {
            if (job->enriched_from_cached_identity) {
                ...
                fetch_result_destroy(&job->response);
                job->enriched_from_cached_identity = false;
                youtube_watch_config_cache.valid = false;
                job->client_index = 0;
                job->phase = YOUTUBE_RESOLVE_PHASE_DIRECT_START;
            } else {
                youtube_resolve_job_fail(
