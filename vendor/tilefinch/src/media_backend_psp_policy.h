...
/* Startup waits for five seconds of runway. On the PSP each googlevideo TLS
   handshake costs about two seconds of CPU, and the video and audio
   connections are both opened as playback starts; with only three seconds in
   hand the first seconds of playback competed with that burst and audio
   stuttered. A startup whose fixed windows are already full (fill idle)
   starts from the ordinary two-second target instead of waiting forever. */
#define PSP_MEDIA_BUFFER_STARTUP_TARGET_US 5000000u
/* Audio reserve that admits the optional third video window. */
#define PSP_MEDIA_LOOKAHEAD_AUDIO_RESERVE_US 3000000u
    ...
    bool separate_audio, uint64_t audio_ahead_us)
{
    return (!separate_audio
            || audio_ahead_us >= PSP_MEDIA_LOOKAHEAD_AUDIO_RESERVE_US)
        ? 2u : 1u;
}
    ...
    PspMediaBufferPolicyInput input)
{
    ...
    bool runway = input.network_ahead_us >= decision.target_us
        || (input.startup && !input.fill_pending
