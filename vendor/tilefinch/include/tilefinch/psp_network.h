...
typedef struct {
    ...
    /* Foreground connections hand association to the firmware's own
       network-connection dialog instead of calling sceNetApctlConnect with a
       guessed profile. The ladder stops in CONNECTING with dialog_pending set;
       the owner runs psp_network_run_connect_dialog() outside any cooperate
       scope and then commits the outcome with psp_network_finish_dialog(). */
    bool connect_via_dialog;
    bool dialog_pending;
} PspNetwork;
...
PspNetworkStatus psp_network_pump(PspNetwork *network,
                                  uint64_t timeout_us);
/* Run the PSP system "connect to an access point" dialog. It owns the GE and
   the display until it closes. Returns the dialog's result code (0 success,
   1 cancelled by the user) or a negative firmware error from InitStart. */
int psp_network_run_connect_dialog(void);

typedef enum {
    PSP_BOOT_CONNECT_READY = 0,
    /* The user chose not to retry; the caller should quit. */
    PSP_BOOT_CONNECT_DECLINED,
    /* The network stack could not start; a firmware error was shown. */
    PSP_BOOT_CONNECT_FAILED
} PspBootConnectResult;

/* Start the network services and connect through the PSP system Wi-Fi
   dialog, offering a retry prompt until connected or declined. Must run on
   the main thread before any other code draws or starts networking.
   profile_out receives the saved profile the dialog joined (0 if unknown). */
PspBootConnectResult psp_network_boot_connect(int *profile_out);
/* Commit a dialog outcome: READY when APCTL holds an address, otherwise
   CANCELLED or FAILED. The connected profile index is recovered by name so
   later rejoins reconnect to what the user picked. */
